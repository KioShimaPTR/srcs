...
void CHARACTER::EncodeInsertPacket(LPENTITY entity)
{
	...
	if (IsPC() == true || m_bCharType == CHAR_TYPE_NPC)
	{
...
#ifdef ENABLE_TITLE_SYSTEM
		addPacket.bTitle = m_bTitle;
#endif
#ifdef ENABLE_COSTUME_EFFECT_SYSTEM
		LPITEM item = GetWear(WEAR_COSTUME_BODY);
		addPacket.dwCostumEvo = item ? item->GetSocket(1) : 0;
#endif
		...
		if (g != NULL)
		{
			auto getMember = g->GetMember(GetPlayerID());
			if (getMember)
			{
...
void CHARACTER::UpdatePacket()
{
...
#ifdef ENABLE_TITLE_SYSTEM
	pack.bTitle = m_bTitle;
#endif
	pack.bPKMode = m_bPKMode;
	pack.dwMountVnum = GetMountVnum();
#ifdef ENABLE_COSTUME_EFFECT_SYSTEM
	LPITEM item = GetWear(WEAR_COSTUME_BODY);
	pack.dwCostumEvo = item ? item->GetSocket(1) : 0;
#endif
#ifdef ENABLE_GUILD_LEADER_SYSTEM
	pack.bMemberType = 0;
	auto guild = GetGuild();
	if (guild != NULL)
	{
		auto getMember = guild->GetMember(GetPlayerID());
		if (getMember)
		{
			if (getMember->grade == GUILD_LEADER_GRADE)
				pack.bMemberType = 1;
...
void CHARACTER::SetPlayerProto(const TPlayerTable* t)
{
	...
	//GMÀÏ¶§ º¸È£¸ðµå
#ifdef ENABLE_HUNTER_SYSTEM
	if (!test_server && GetGMLevel() > GM_PLAYER)
#else
...
#ifndef ENABLE_EXTENDED_PET_SYSTEM
...
void CHARACTER::ResetPoint(int iLv)
{
	if (GetMapIndex() >= 10000)
	{
		...
		{
			ChatPacket(CHAT_TYPE_INFO, "Burada statü sýfýrlayamazsýn.");
			return;
		}
	}

	if (get_global_time() < GetQuestFlag("statu.engel"))
	{
		ChatPacket(CHAT_TYPE_INFO, "Statü sýfýrladýktan sonra tekrar sýfýrlamak için 3 dakika beklemelisin. ");
		return;
	}
	...
	SetQuestFlag("statu.engel", get_global_time() + 60 * 3);
}
...
#ifdef ENABLE_SHIP_WAR_SYSTEM
	if (CShipWarManager::Instance().IsShipMap(GetMapIndex()))
		return;
#endif
...
void CHARACTER::PetSummon(LPITEM petItem)
{
	...
	if (petItem->IsPetItem())
		petSystem->Summon(mobVnum, petItem, " - Evcil Hayvan", false);
}
...
#ifdef ENABLE_GUILD_WAR_RENEWAL_SYSTEM
bool CHARACTER::IsDisabledWarMap(LPITEM item)
{
	if (GetMapIndex() >= 10000)
	{
		if (CWarMapManager::instance().IsWarMap(GetMapIndex()))
			...
			if (pMap)
			{
				if (GetGuild())
				{
					if (item->GetType() == ITEM_COSTUME)
					{
						...
						{
							if (pMap->IsEquipmentCostume(GetGuild()->GetID()) == false)
								return false;
						}
						else if (item->GetSubType() == COSTUME_MOUNT)
						{
							if (pMap->IsMountRiding(GetGuild()->GetID()) == false)
								return false;
						}
					}
					else if (item->GetType() == ITEM_UNIQUE)
					{
						if (item->GetSubType() == USE_PET)
						{
							if (pMap->IsSummonPet(GetGuild()->GetID()) == false)
								return false;
						}
						else if (item->GetSubType() == USE_WING)
						{
							if (pMap->IsEquipmentCostume(GetGuild()->GetID()) == false)
								return false;
						}
