...
int CInputMain::Chat(LPCHARACTER ch, const char* data, size_t uiBytes)
{
...
#ifdef ENABLE_SHIP_WAR_SYSTEM
	if (CShipWarManager::Instance().IsShipMap(ch->GetMapIndex()) && !CShipWarManager::Instance().IsShipWarChatActive() && !ch->IsGM())
	{
		if (CHAT_TYPE_TALKING == pinfo->type)
			return iExtraLen;
	}
#endif
	...
	if (CHAT_TYPE_SHOUT == pinfo->type)
	{
		...
		len = snprintf(chatbuf, sizeof(chatbuf), "%s  [Lv.%d] - [CH%d] - %s : %s", kingdoms[ch->GetEmpire() - 1], ch->GetLevel(), g_bChannel, ch->GetName(), buf);
...
void CInputMain::AnswerMakeGuild(LPCHARACTER ch, const char* c_pData)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "<Hata> Yetersiz yang!");
		return;
	}

	if (ch->GetLevel() < 40)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "<Hata> Yetersiz level!");
		return;
	}
	...
	if (dwGuildID)
	{
		...
		std::vector<const char*> kingdoms = { "Shinsoo", "Chunjo", "Jinno" };
		char szNotice[CHAT_MAX_LEN + 1];
		snprintf(szNotice, sizeof(szNotice), "<Lonca> |cffffcc00[%s]|r Loncasý %s Tarafýndan %s Krallýðýnda kuruldu.", cp.name, ch->GetName(), kingdoms[ch->GetEmpire() - 1]);
